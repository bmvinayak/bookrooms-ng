...
export class ManageRentalComponent implements OnInit {
  ...
  ngOnInit() {
    const rentalObserable = this.rentalService.getUserRentalItems();
  ...
  deleteRentalItem(rentalItemId: string) {
    ...
    rentalObserable.subscribe(
      () => {
        ...
        this.toastrService.success('Your Rental has been deleted.', 'Success');
      },
      (errorResponse: HttpErrorResponse) => {
        this.errors = errorResponse.error.errors;
        this.rentalDeleteIndex= undefined;
        this.toastrService.error(this.errors[0].title + ': ' + this.errors[0].detail, 'Error');
      },
      () => {
      })
  }
